function scrDrawMobileControls(plr = noone, scale = UberCont.opt_controls_scale) {
    ...
    with JoystickMove {
        ...
        if UberCont.opt_simplify {
            ...
            draw_sprite_ext(sprControlJoystick, 0, x, y, scale, scale, 0, c_white, 1)
            draw_sprite_ext(sprControlJoystick, 1, x + xx, y + yy, scale, scale, 0, c_white, 1)
        }
    }
    ...
    with JoystickAttack {
        ...
        var xx = lengthdir_x(dis, dir) - 1
        var yy = lengthdir_y(dis, dir) - 1
    ...
    with ButtonSwap {
        ...
        if !UberCont.opt_simplify {
            draw_circle_part(x, y, rad * m, width, 1 - reload / UberCont.wep_load[wep])
    ...
    if save_get_val("controls", "volumecontrol", 1) {
