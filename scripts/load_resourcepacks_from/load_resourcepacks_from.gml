function load_resourcepacks_from(loadpath) {
    ...
    for (var i = 0; i < array_length(list); i++) {
        ...
        try {
            ...
            scr_log_push("! meta.json is malformed or doesn't exist. Generating template...", c_yellow)

	        pack[$ "name"] = _name
	        pack[$ "descriptionShort"] = scrSavedataGetUsername() + "'s resourcepack"
		...
		if file_exists(packdir + "githubdata.json") {
		    try {
		        ...
		        scr_log_push("[!] githubdata.json parse failed: " + e.message, c_yellow)
		    }
		}
		else local = true
