...
if instance_exists(target) {
    if (main == 1) {
        if (drawspr == sprTechnoMancerAppear) {
		...
		else if (instance_nearest(target.x, target.y, TechnoMancer) == id) {
            revive = 0
            
			if instance_exists(Corpse) {
                dir = instance_nearest(x, y, Corpse)
				
                if !collision_line(x, y, dir.x, dir.y, Wall, 0, 0) {
					revive = 1
				}
			}
			
            if (revive && (random(5) < 6)) {
            ...
            if drawspr != sprTechnoMancerInactive {
